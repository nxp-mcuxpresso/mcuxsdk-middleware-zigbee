...
# Default path is empty, so that it will be searched in the $PATH env var
ARM_TOOLCHAIN_PATH=""
...
def parse_sections(file):
    ...
    objdump = subprocess.check_output([ARM_TOOLCHAIN_PATH + OBJDUMP, '-h', file])
...
def get_symbol_addr(file, symbol):
    objdump = subprocess.check_output([ARM_TOOLCHAIN_PATH + OBJDUMP, '-t', file])
    objdump = objdump.decode('utf-8')

    symbol_re = re.compile(
        rf'(?P<addr>\d+).*\b{symbol}\b'
    )

    for match in re.finditer(symbol_re, objdump):
        symbol_dict = match.groupdict()
        addr = int('0x' + symbol_dict['addr'], 16)
        logger.debug(f'Found symbol {symbol}, addr: {hex(addr)}')
        return addr
    return None
...
def gen_binary(elf):
    ...
    subprocess.run([ARM_TOOLCHAIN_PATH + OBJCOPY, '-O', 'binary', elf, temp_bin.name], check = True)
    temp_bin.seek(0)
    data = temp_bin.read()
...
def main():
    ...
    parser.add_argument('-d', '--device', help='Device type', choices=['K32W1'])
    ...
    parser.add_argument('--signed', help='Input file that has been externally signed (with mcuboot imgtool for instance). \
                                          This file will get populated with OTA headers, no extra SB3 file will be generated.')
    ...
    parser.add_argument('--enc_offset', type=int, help='Offset where the encryption should start.')
    ...
    parser.add_argument("-c", '--CompilerTools', help="Path to a arm-gcc-none-eabi compatible toolchain.")
    ...
    if args.signed != None:
        signed_file_input = os.path.abspath(args.signed)
        logger.info(f'Input signed binary: {signed_file_input}')
    else:
        signed_file_input = None

    if args.CompilerTools != None:
        logger.info(f'Compiler Tools Path: {args.CompilerTools}')
        if os.name == 'nt':
    ...
    if not args.enc_offset:
        if signed_file_input != None:
            flash_start = get_symbol_addr(temp_elf_file, "_flash_start")
            enc_start = get_symbol_addr(temp_elf_file, "_enc_start")
            args.enc_offset = enc_start - flash_start
        else:
            args.enc_offset = 0
        logger.debug(f'Using encryption offset {hex(args.enc_offset)}')
    ...
    if args.embed:
        ...
        if signed_file_input != None:
            ota_file_temp = tempfile.NamedTemporaryFile(delete=False)
            copy_file(signed_file_input, ota_file_temp.name)
            ota_file_data = ota_file_temp.read()
            logger.debug(f'Pre-signed binary, bin len: {len(ota_file_data)}')
        else:
        ...
        ota_file_data_encrypted = ota_file_data[args.enc_offset:]

        if args.encrypt:
            ota_file_data_encrypted = encryptFlashData(binascii.unhexlify(args.nonce), binascii.unhexlify(args.key), ota_file_data[args.enc_offset:])
        else:
            logger.debug(f'Encryption skipped')

        if not signed_file_input:
            size_before = len(ota_file_data)
            ...
            size_before = len(ota_file_data)
            ota_file_data = ota_file_data[:args.enc_offset] + ota_file_data_encrypted
            logger.debug(f'Encrypted binary, bin len: {len(ota_file_data)}, diff: {len(ota_file_data) - size_before}')
