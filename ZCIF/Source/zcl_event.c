    ...
    case eFRAME_TYPE_COMMAND_ACTS_ACCROSS_ENTIRE_PROFILE:
        {
            ...
            else if( psZCL_EndPointDefinition != NULL )
            {
                ...
                // and even if it is present, check if the cluster cmd handling is overridden by the app
 ...
 ** even if the cluster is present
 **
 ** PARAMETERS:               Name                    Usage
 ...
 **
 ** DESCRIPTION:
 ** Checks whether ZCL general command handling needs to be overridden to not
 ** process the received message even if the cluster instance exist locally
 **
 ** PARAMETERS:               Name                    Usage
 **
 ** RETURN:
