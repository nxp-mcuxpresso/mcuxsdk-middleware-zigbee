...
/****************************************************************************
**
** NAME:       vZCL_HandleDiscoverAttributesExtendedResponse
**
** DESCRIPTION:
**
**
    ...
    tsZCL_ClusterInstance      *psClusterInstance)
{
    ...
    sZCL_CallBackEvent.eEventType = E_ZCL_CBET_DISCOVER_INDIVIDUAL_ATTRIBUTE_EXTENDED_RESPONSE;
    ...
    if (sZCL_CallBackEvent.eEventType != E_ZCL_CBET_ERROR)
    {
        ...
        u8NumberOfAttributes = (u16payloadSize - u16inputOffset - 1)/4;

        // check for junk - carry on if we can
        bMessageError=FALSE;
        if((u16payloadSize - u16inputOffset- u8NumberOfAttributes*4 - 1)!= 0)
        {
            bMessageError=TRUE;
        }
        ...
        // M$ enum issue
        sZCL_CallBackEvent.uMessage.sAttributeDiscoveryExtenedResponse.eAttributeDataType = 0;
        ...
        u16inputOffset += u16ZCL_APduInstanceReadNBO(pZPSevent->uEvent.sApsDataIndEvent.hAPduInst,
            u16inputOffset,
            E_ZCL_UINT8,
            &sZCL_CallBackEvent.uMessage.sAttributeDiscoveryExtenedResponse.bDiscoveryComplete);
        ...
        for(i=0; i<u8NumberOfAttributes; i++)
        {
            ...
            u16inputOffset += u16ZCL_APduInstanceReadNBO(pZPSevent->uEvent.sApsDataIndEvent.hAPduInst,
                u16inputOffset,
                E_ZCL_ATTRIBUTE_ID,
                &sZCL_CallBackEvent.uMessage.sAttributeDiscoveryExtenedResponse.u16AttributeEnum);
            // read type
            u16inputOffset += u16ZCL_APduInstanceReadNBO(pZPSevent->uEvent.sApsDataIndEvent.hAPduInst,
                u16inputOffset,
                E_ZCL_UINT8,
