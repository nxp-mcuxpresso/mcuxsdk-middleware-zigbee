...
#define gAppMaxQueueSize                (6)     /* Size of queue used for Zigbee Tx messages */
...
typedef struct
{
    ...
    messaging_t msgQueue; /* Message queue */
...
static void UartStreamFlushTimerZigbeeCallback(void *pData);
...
__unused static button_status_t BleApp_HandleKeys0(void *pButtonHandle, button_callback_message_t *pMessage, void *pCallbackParam);
...
SERIAL_MANAGER_WRITE_HANDLE_DEFINE(s_writeHandle);
...
void BluetoothLEHost_AppInit(void)
{
   ...
   // (void)BUTTON_InstallCallback((button_handle_t)g_buttonHandle[1], BleApp_HandleKeys1, NULL);
    ...
    /* Initialize packetToSend */
    packetToSend.pMsgCurrent=NULL;
    packetToSend.msgQueueSize = 0;
    MSG_QueueInit(&packetToSend.msgQueue);
...
void BleApp_Stop(void)
{
    ...
    if (!isBleRunning)
    {
        /* Notify the zigbee app */
        (void)App_PostCallbackMessage(APP_ZigbeeProcessBleEvent, (void *) eBleNotRunningEvent);
    }
}
    ...
    switch (pAdvertisingEvent->eventType)
    {
        case gAdvertisingStateChanged_c:
        {
            ...
            else
            {
                ...
                /* Notify the zigbee app */
                (void)App_PostCallbackMessage(APP_ZigbeeProcessBleEvent, (void *) eBleAdvStopEvent);
            }
        }
        break;
...
)
{
    switch (pConnectionEvent->eventType)
    {
        ...
        case gConnEvtDisconnected_c:
        {
            ...
            {
                msgBuffer_t *pMsg = MSG_QueueRemoveHead(&packetToSend.msgQueue);
                if (pMsg != NULL)
                {
            ...
            {
                (void)App_PostCallbackMessage(APP_ZigbeeProcessBleEvent, (void *) eBleDisconnectionEvent);
            }
        }
        break;
 ...
 *
 * \param[in]    pRecvStream        Pointer to the received stream.
 * \param[in]    streamSize         The number of bytes in the stream.
 ********************************************************************************** */
static bleResult_t BleApp_SendUartStream
(
    uint8_t *pRecvStream,
    uint8_t streamSize
)
{
    ...
    bleResult_t result = gBleUnavailable_c;
         ...
         mAppRunning_c == maPeerInformation[mPeerId].appState)
     {
         ...
         result = GattClient_WriteCharacteristicValue(maPeerInformation[mPeerId].deviceId, &characteristic,
...
)
{
    ...
    /* Allocate buffer for Zigbee shell command */
    pBuffer = MEM_BufferAlloc(streamLength);

    if (pBuffer != NULL)
    {
        FLib_MemCpy(pBuffer, pStream, (uint32_t)streamLength);
        APP_ZigbeeShellCommand(pBuffer, streamLength);
...
/*! *********************************************************************************
 * \brief        Timer handler for flushing the UART in context of Zigbee shell
 *
 * \param[in]    pData              Pointer to the parameters.
 ********************************************************************************** */
static void UartStreamFlushTimerZigbeeCallback
(
    void *pData
)
{
    ...
    packetToSend.pMsgCurrent = NULL;
    TM_Stop(mUartStreamFlushTimerId);
}
/*! *********************************************************************************
...
/*! *********************************************************************************
* \brief        Send through Bluetooth data from Zigbee application
*
...
uint32_t Serial_CustomSendData(uint8_t *pData, uint32_t size)
{
    ...
    uint8_t *pRead = pData;
    uint8_t mPeerId = 0; /* send only on 1st peer */

    TM_Stop(mUartStreamFlushTimerId);

    if (maPeerInformation[mPeerId].appState == mAppRunning_c)
    {
        while (NoOfBytes > 0)
        {
            if (packetToSend.pMsgCurrent != NULL)
            ...
            {
                pMsg = MSG_Alloc(sizeof(msgBuffer_t));
                if (pMsg == NULL)
                {
                    status = 1;
                    Serial_Print("Alloc FAILURE\r\n", gAllowToBlock_d);
                    break;
                }
        ...
        {
            /* Restart flush timer */
            (void)TM_InstallCallback((timer_handle_t)mUartStreamFlushTimerId, UartStreamFlushTimerZigbeeCallback, NULL);
            (void)TM_Start((timer_handle_t)mUartStreamFlushTimerId,
                         (uint8_t)kTimerModeLowPowerTimer | (uint8_t)kTimerModeSingleShot, mAppUartFlushIntervalInMs_c);
        }
    }
    else
    {
        ...
        {
            MEM_BufferFree(packetToSend.pMsgCurrent->pBuffer);
            MSG_Free(packetToSend.pMsgCurrent);
            packetToSend.pMsgCurrent = NULL;
        }
    }
...
/*! *********************************************************************************
 * \brief    Function used to queue messages from Zigbee
 *
 ********************************************************************************** */
static bool_t BleApp_MsgQueue(msgBuffer_t *pMsg)
{
    ...
    {
        MSG_QueueAddTail(&packetToSend.msgQueue, pMsg);
        packetToSend.msgQueueSize++;
...
/*! *********************************************************************************
 * \brief    Function handling the queued Zigbee messages and send them over Uart
 *
 ********************************************************************************** */
static void BleApp_SendStreamCb(void *pParam)
{
    ...
    {
        msgBuffer_t *pMsg = MSG_QueueGetHead(&packetToSend.msgQueue);
        ...
        pMsg = MSG_QueueRemoveHead(&packetToSend.msgQueue);
        if (pMsg != NULL)
        {
             if (pMsg->pBuffer)
                 MEM_BufferFree(pMsg->pBuffer);
