...
PRIVATE void vAPPSendZPSData (uint8 PktType, uint8 *pu8Buffer, uint16 *pu16ClusterId, uint8 *pu8SeqNum, uint8 *pu8Status);
...
//  }
    ...
    case E_SL_MSG_INTERPAN_DATA_REQ:
    {
        ...
        if (hAPduInst)
        {
            ...
            /* copy u8DstAddrMode */
            u8DstAddrMode = au8LinkRxBuffer[u16Index++];
            ...
            /* copy u8Handle */
            u8Handle = au8LinkRxBuffer[u16Index++];
    ...
    case E_SL_MSG_SET_DEVICE_PERMISSIONS:
    {
        ...
        u8DevicePermissions = au8LinkRxBuffer[u16Index++];
    ...
    case E_SL_MSG_SET_REPROVISSION_DATA:
    {
        ...
        u8Permissions = au8LinkRxBuffer[u16Index++];
    ...
    case E_SL_MSG_ADD_REPLACE_LINK_KEY:
    {
        ...
        /* copy au8Key
         * endian memcpy ok byte arrray to byte array */
        memcpy(au8Key, &au8LinkRxBuffer[u16Index], ZPS_SEC_KEY_LENGTH);
        u16Index += ZPS_SEC_KEY_LENGTH;

        /* copy eKeyType */
        // TODO: Replace with ZNC_BUF_U8_UPD
        u8KeyType = au8LinkRxBuffer[u16Index++];
    ...
    case E_SL_MSG_SET_INIT_SEC_STATE:
    {
        ...
        /* copy eKeyState */
        eKeyState = au8LinkRxBuffer[u16Index++];
        ...
        /* copy u8KeySeqNum */
        u8KeySeqNum = au8LinkRxBuffer[u16Index++];
    ...
    case E_SL_MSG_SET_ENDPOINT_STATE:
    {
        ...
        /* copy u8Endpoint */
        u8Endpoint = au8LinkRxBuffer[u16Index++];

        /* copy u8EPState */
        u8EPState = au8LinkRxBuffer[u16Index++];
    ...
    case E_SL_MSG_GET_KEY_TABLE_ENTRY:
    {
        ...
        {
            /* copy query type */
            u8QueryType = au8LinkRxBuffer[u16Index++];
    ...
    case E_SL_MSG_SET_KEY_TABLE_ENTRY:
    {
        ...
        else
        {
            ...
            /* memcpy ok byte array to byte array */
    ...
    case E_SL_MSG_GET_CLUSTER_DISCOVERY_STATE: {
        ...
        /* copy bOutput */
        bOutput = au8LinkRxBuffer[u16Index++];
    ...
    case E_SL_MSG_SET_CLUSTER_DISCOVERY_STATE: {
        ...
        /* copy bDiscoverable */
        bDiscoverable = au8LinkRxBuffer[u16Index++];
    ...
    case E_SL_MSG_SET_APS_FRAME_COUNTER:
    {
        ...
        /* copy bIsInOrOut */
        bInFC = au8LinkRxBuffer[u16Index++];
    ...
    case (E_SL_MSG_UNBIND): {
        ...
        u8SrcEp = au8LinkRxBuffer[u16Index++];
        ...
        if (u8DstAddrMode == 0x3) {
            ...
            u8DstEp = au8LinkRxBuffer[u16Index++];
        } else {
            uDstAddress.u16Addr = ZNC_RTN_U16_OFFSET( au8LinkRxBuffer, u16Index, u16Index );
            u8DstEp = 0;
        }
    ...
    case E_SL_MSG_ZDO_BIND:
    {
        ...
        u8SrcEndpoint = au8LinkRxBuffer[u16Index++];
        u8DstEndpoint = au8LinkRxBuffer[u16Index++];
        u8DstAddrMode = au8LinkRxBuffer[u16Index++];
    ...
    case (E_SL_MSG_RESET): {
        ...
        //DBG_vPrintf(TRUE, "App_vSoftwareReset();\n");
        RESET_SystemReset();
        return;
    }
        break;
    ...
    case (E_SL_MSG_MATCH_DESCRIPTOR_REQUEST): {
        ...
        u8InClusterCount = au8LinkRxBuffer[u16Index++];
        ...
        u8OutClusterCount = au8LinkRxBuffer[u16Index++];
    ...
    case (E_SL_MSG_SIMPLE_DESCRIPTOR_REQUEST): {
        ...
        /* copy bIsExtAddress */
        bIsExtAddress = au8LinkRxBuffer[u16Index++];
        ...
        /* copy u8Endpoint */
        sZdpSimpleDescReq.u8EndPoint = au8LinkRxBuffer[u16Index++];
    ...
    case (E_SL_MSG_NETWORK_ADDRESS_REQUEST): {
        ...
        uint8 u8RequestType = au8LinkRxBuffer[u16Index++];
        uint8 u8StartIndex = au8LinkRxBuffer[u16Index++];
    ...
    case E_SL_MSG_GET_NEIGHBOR_TABLE_ENTRY:
    {
        ...
        else
        {
            ...
            au8values[u8TxLength++] = sEntry.u8TxFailed;
            au8values[u8TxLength++] = sEntry.u8LinkQuality;

            au8values[u8TxLength++] = sEntry.u8Age;
            au8values[u8TxLength++] = sEntry.u8ZedTimeoutindex;
            au8values[u8TxLength++] = sEntry.i8TXPower;
            au8values[u8TxLength++] = sEntry.u8MacID;

            au8values[u8TxLength++] = sEntry.uAncAttrs.au8Field[0];
            au8values[u8TxLength++] = sEntry.uAncAttrs.au8Field[1];
    ...
    case E_SL_MSG_GET_NEIGHBOR_TABLE_ENTRY_BY_ADDRESS:
    {
        ...
        /* copy u8AddrMode */
        u8AddrMode = au8LinkRxBuffer[u16Index++];
    ...
    case E_SL_MSG_GET_ROUTING_TABLE_ENTRY:
    {
        ...
        else
        {
            ...
            u16Field = (sEntry.uAncAttrs.au8Field[0] << 8) | sEntry.uAncAttrs.au8Field[1];
            //ZNC_BUF_U32_UPD( &au8values[u8TxLength],  *((uint32*)sEntry.uAncAttrs.au8Field) , u8TxLength );
            ZNC_BUF_U16_UPD( &au8values[u8TxLength],  u16Field , u8TxLength );
    ...
    case E_SL_MSG_TRANSPORT_NETWORK_KEY:
    {
        ...
        /* copy bUseParent */
        bUseParent = au8LinkRxBuffer[u16Index++];
    ...
    case E_SL_MSG_GET_SIMPLE_DESCRIPTOR:
    {
        ...
        //ZNC_BUF_U32_UPD( &au8values[u8TxLength],  , u8TxLength );
    ...
    case E_SL_MSG_SET_NWK_INTERFACE_REQ:
    {
        ...
        u8InterfaceIndex = au8LinkRxBuffer[u16Offset++];

        bState = au8LinkRxBuffer[u16Offset++];

        bRoutersAllowed = au8LinkRxBuffer[u16Offset++];

        u32ChanToUse = ZNC_RTN_U32_OFFSET( au8LinkRxBuffer, u16Offset, u16Offset );

        u8ChanCount = au8LinkRxBuffer[u16Offset++];
    ...
    case E_SL_MSG_SEND_MGMT_NWK_UNS_ENH_UPDATE_NOTIFY:
    {
        ...
        if (PDUM_INVALID_HANDLE != hAPduInst)
        {
            ...
            sMgmtNwkUnSolicitedUpdateNotify.u8PeriodOfTimeForResults = au8LinkRxBuffer[u8Location++];
    ...
    case E_SL_MSG_SET_RESTORE_POINT:
    {
        ...
        asSecMatSet[0].u8KeySeqNum = au8LinkRxBuffer[u16Index++];

        asSecMatSet[0].u8KeyType =  au8LinkRxBuffer[u16Index++];

        /*  byte copy ok*/
        memcpy( &asSecMatSet[1].au8Key, &au8LinkRxBuffer[u16Index], 16);
        u16Index += 16;

        asSecMatSet[1].u8KeySeqNum =  au8LinkRxBuffer[u16Index++];;

        asSecMatSet[1].u8KeyType = au8LinkRxBuffer[u16Index++];
        ...
        sRestorePoint.u8KeyType = au8LinkRxBuffer[u16Index++];
        u16Index++;
    ...
    case E_SL_MSG_SET_MAC_ADDR_TABLE:
    {
        ...
        //memcpy(psNib->sTbl.pu64AddrExtAddrMap, &au8LinkRxBuffer[1],    sizeof(uint64)*u8MacAddTableSize);
        *psNib->sTbl.pu64AddrExtAddrMap = ZNC_RTN_U64( au8LinkRxBuffer, 1 );
    ...
    case E_SL_MSG_START_ROUTER:
    {
        ...
        sStartParms.u8KeyIndex       = au8LinkRxBuffer[u16Index++];
        sStartParms.u8LogicalChannel = au8LinkRxBuffer[u16Index++];

        // todo this needs checking for endian
        sStartParms.u16NwkAddr = (uint16)(au8LinkRxBuffer[u16Index++]) << 8;
        sStartParms.u16NwkAddr += (uint16)au8LinkRxBuffer[u16Index++];

        /* TODO needs endian check */
        sStartParms.u16PanId = (uint16)(au8LinkRxBuffer[u16Index++]) << 8;
        sStartParms.u16PanId += (uint16)au8LinkRxBuffer[u16Index++];
...
PRIVATE void vAPPSendZPSData (uint8 PktType, uint8 *pu8Buffer, uint16 *pu16ClusterId, uint8 *pu8SeqNum, uint8 *pu8Status)
{
    ...
    {
        Params.u8BrdAddrMode = pu8Buffer[u16Index++];
    }
