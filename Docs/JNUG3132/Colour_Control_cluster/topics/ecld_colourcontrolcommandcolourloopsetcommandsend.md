...
This function sends a Colour Loop Set command to instruct a device to configure the movement of the ‘enhanced current hue’ attribute value around the colour loop corresponding to the CIE colour ‘triangle’. The configured movement can be started in either direction and for a specific duration. The start hue, direction and duration are specified in the payload of the command \(see [Section 31.7.2](custom_command_payloads.md#id_9e6d37c2-c3fb-4f2f-83d7-bbac06d8c829)\). The ‘current hue’ attribute is also moved through values based on the ‘enhanced current hue’ value.
