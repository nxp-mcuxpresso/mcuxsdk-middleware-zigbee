...
This function sends a Move to Hue command to instruct a device to move its ‘current hue’ attribute to a target hue value in a continuous manner within a given time. The hue value, direction and transition time are specified in the payload of the command \(see [Section 31.7.2](custom_command_payloads.md#id_9e6d37c2-c3fb-4f2f-83d7-bbac06d8c829)\).
...
This function invokes the ZigBee PRO stack function to transmit the data. In case an error is returned, call the **eZCL\_GetLastZpsError\(\)** function to get the error.
