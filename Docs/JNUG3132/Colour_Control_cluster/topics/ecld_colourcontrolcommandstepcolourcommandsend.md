...
This function sends a Step Colour command to instruct a device to change its ‘current x’ and ‘current y’ attribute values by a specified ‘step’ value for each attribute in a continuous manner within a given time \(where x and y are the chromaticities from the CIE xyY colour space\). The step sizes and transition time are specified in the payload of the command \(see [Section 31.7.2](custom_command_payloads.md#id_9e6d37c2-c3fb-4f2f-83d7-bbac06d8c829)\), and each step size can be positive \(increase\) or negative \(decrease\).
