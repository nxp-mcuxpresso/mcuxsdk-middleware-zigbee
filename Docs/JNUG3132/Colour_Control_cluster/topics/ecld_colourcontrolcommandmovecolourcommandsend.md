...
This function sends a Move Colour command to instruct a device to move its ‘current x’ and ‘current y’ attribute values at a specified rate for each attribute for an indefinite time \(where x and y are the chromaticities from the CIE xyY colour space\). The rates are specified in the payload of the command \(see [Section 31.7.2](custom_command_payloads.md#id_9e6d37c2-c3fb-4f2f-83d7-bbac06d8c829) and each rate can be positive \(increase\) or negative \(decrease\).
