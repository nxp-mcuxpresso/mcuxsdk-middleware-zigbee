# eCLD\_IdentifyCommandIdentifyQueryRequestSend
...
A device which receives this command generates a callback event on the endpoint on which the Identify cluster is registered. If the receiving device is in identification mode, it returns a response containing the amount of time for which it continues in this mode \(see [Section 11.7.3](custom_command_responses.md#id_5df15f8e-cd2d-4e19-99d6-47f0e4f63e54)\).
