# eCLD\_GroupsCommandAddGroupIfIdentifyingRequestSend
...
This function sends an Add Group If Identifying command to a remote device, requesting that the specified endpoints on the target device be added to a particular group on the condition that the remote device is identifying itself. The group ID/address and name \(if supported\) are specified in the payload of the message, and must be added to the Group table on the target node along with the associated endpoint numbers. The identifying functionality is controlled using the Identify cluster \(see [Chapter 11](../../Identify_cluster/topics/identify_cluster.md#id_8d39ccb7-5ec3-439b-af2c-8b6105768b6d)\).
