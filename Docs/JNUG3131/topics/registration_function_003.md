...
The following **eZLO\_RegisterOccupancySensorEndPoint\(\)** function is the endpoint registration function for an Occupancy Sensor device.
...
This function is used to register an endpoint which supports an Occupancy Sensor device. The function must be called after **eZCL\_Initialise\(\)**.
...
-   *psDeviceInfo*: Pointer to the structure that acts as storage for all variables related to the device being registered on this endpoint \(see [Section 3.8.2](device_structure_007.md#ID_C8102A52-389D-4151-A406-BE32814A7EEA)\). This function sets the `sEndPoint` and `sClusterInstance` fields of this structure and these fields must not be directly written to by the application.
