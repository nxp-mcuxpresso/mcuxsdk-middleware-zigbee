...
The following **eZLO\_RegisterOnOffBallastEndPoint\(\)** function is the endpoint registration function for an On/Off Ballast device.
...
This function is used to register an endpoint which supports an On/Off Ballast device. The function must be called after **eZCL\_Initialise\(\)**.
...
-   *psDeviceInfo*: Pointer to the structure that acts as storage for all variables related to the device being registered on this endpoint \(see [Section 3.9.2](device_structure_014.md#ID_7378563B-EC9E-4B87-988F-FF8053647E49)\). The `sEndPoint` and `sClusterInstance` fields are set by this register function for internal use and must not be written to by the application.
