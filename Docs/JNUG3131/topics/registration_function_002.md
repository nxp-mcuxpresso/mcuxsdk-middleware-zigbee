...
The following **eZLO\_RegisterOnOffSensorEndPoint\(\)** function is the endpoint registration function for an On/Off Sensor device.
...
This function is used to register an endpoint which supports an On/Off Sensor device. The function must be called after **eZCL\_Initialise\(\)**.
...
-   *psDeviceInfo*: Pointer to the structure that acts as storage for all variables related to the device being registered on this endpoint \(see [Device structure](device_structure_019.md#ID_F401FE2D-08CD-41B5-9B80-EFFFBD47972D)\). The `sEndPoint` and `sClusterInstance` fields are set by this register function for internal use and must not be written to by the application.
