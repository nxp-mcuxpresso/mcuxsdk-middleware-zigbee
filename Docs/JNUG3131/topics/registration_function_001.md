...
The following **eZLO\_RegisterLightLevelSensorEndPoint\(\)** function is the endpoint registration function for a Light Level Sensor device.
...
This function is used to register an endpoint which supports a Light Level Sensor device. The function must be called after **eZCL\_Initialise\(\)**.
