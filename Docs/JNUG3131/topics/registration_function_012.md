...
The following **eZLO\_RegisterNonColourControllerEndPoint\(\)** function is the endpoint registration function for a Non-Colour Controller device.
...
This function is used to register an endpoint which supports a Non-Colour Controller device. The function must be called after **eZCL\_Initialise\(\)**.
...
-   *psDeviceInfo*: Pointer to the structure that acts as storage for all variables related to the device being registered on this endpoint \(see [Section 3.18.2](device_structure_016.md#ID_DA94A763-8560-42E6-B9F7-FE4919F0749C)\). The `sEndPoint` and `sClusterInstance` fields are set by this register function for internal use and must not be written to by the application.
