...
The following **eZLO\_RegisterOnOffPlugEndPoint\(\)** function is the endpoint registration function for an On/Off Plug-in Unit device.
...
This function is used to register an endpoint which supports an On/Off Plug-in Unit device. The function must be called after **eZCL\_Initialise\(\)**.
...
-   *psDeviceInfo*: Pointer to the structure that acts as storage for all variables related to the device being registered on this endpoint \(see [Section 3.11.2](device_structure_004.md#ID_D345EA38-83A8-42E6-A04D-C43EFB659572)\). The `sEndPoint` and `sClusterInstance` fields are set by this register function for internal use and must not be written to by the application.
...
**Parent topic:**[On/Off Plug-in Unit](../topics/on_off_plug-in_unit.md)
