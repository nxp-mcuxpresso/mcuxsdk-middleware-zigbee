...
The following **eZLO\_RegisterExtendedColourLightEndPoint\(\)** function is the endpoint registration function for an Extended color Light device.
...
This function is used to register an endpoint which supports an Extended color Light device. The function must be called after **eZCL\_Initialise\(\)**.
...
-   *psDeviceInfo*: Pointer to the structure that acts as storage for all variables related to the device being registered on this endpoint \(see [Section 3.14.2](device_structure_006.md#ID_26263C1E-D8AA-4D05-A38E-B47A1CB3F199)\). The `sEndPoint` and `sClusterInstance` fields are set by this register function for internal use and must not be written to by the application.
...
**Parent topic:**[Extended Colour Light](../topics/extended_colour_light.md)
