...
The following **eZLO\_RegisterDimmablePlugEndPoint\(\)** function is the endpoint registration function for a Dimmable Plug-in Unit device.
...
This function is used to register an endpoint which supports a Dimmable Plug-in Unit device. The function must be called after **eZCL\_Initialise\(\)**.
...
-   *psDeviceInfo*: Pointer to the structure that acts as storage for all variables related to the device being registered on this endpoint \(see [Device Structure](device_structure_003.md#ID_9934418E-45FA-4EAB-A457-64616F9A4625)\). The `sEndPoint` and `sClusterInstance` fields are set by this register function for internal use and must not be written to by the application.
