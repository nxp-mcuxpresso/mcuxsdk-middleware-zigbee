...
The following **eZLO\_RegisterColourDimmableLightEndPoint\(\)** function is the endpoint registration function for a color Dimmable Light device.
...
This function is used to register an endpoint that supports a color Dimmable Light device. The function must be called after **eZCL\_Initialise\(\)**.
...
-   *psDeviceInfo*: Pointer to the structure that acts as storage for all variables related to the device being registered on this endpoint \(see [Section 3.3.2](device_structure_001.md#ID_E069EFB9-2590-4E51-B17C-DF11FFDE1691)\). The `sEndPoint` and `sClusterInstance` fields are set by this register function for internal use and must not be written to by the application.
