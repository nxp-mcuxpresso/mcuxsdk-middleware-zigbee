...
The following **eZLO\_RegisterLightSensorEndPoint\(\)** function is the endpoint registration function for a Light Sensor device.
...
This function is used to register an endpoint which supports a Light Sensor device. The function must be called after **eZCL\_Initialise\(\)**.
...
-   *psDeviceInfo*: Pointer to the structure that acts as storage for all variables related to the device being registered on this endpoint \(see [Section 3.7.2](device_structure_018.md#ID_71703D92-BA9C-4D6A-B5AC-F602FC6774E3)\). The `sEndPoint` and `sClusterInstance` fields are set by this register function for internal use and must not be written to by the application.
