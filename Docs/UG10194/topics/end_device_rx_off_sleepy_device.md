# End Device RX Off \(sleepy device\)

The table below lists the LED states for End device RX Off.

| LED1                                                                    | LED2                                           | Remarks                                              |
| ----------------------------------------------------------------------- | ---------------------------------------------- | -------------------------------------------------- |
| OFF                                                                     | OFF                                            | The device is not on the network                   |
| OFF/ON (Current ON/OFF cluster status) or BLINKING ON/OFF (Identifying) | ON                                             | The device is not sleeping and is active           |
| BLINKING ON for 5 seconds and OFF for 1 second                          | BLINKING ON for 5 seconds and OFF for 1 second | The device is going through a sleep and wake cycle |

**Parent topic:**[LED indication table](../topics/led_indication_table.md)

