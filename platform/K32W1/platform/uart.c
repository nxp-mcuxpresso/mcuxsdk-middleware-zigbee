...
bool zbPlatUartInit(void *device)
{
    ...
    /*
        LPUART1_IRQHandler has a default value (0) for priority, 
        but FreeRTOS does not support a lower priority than configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY
        when using a FreeRTOS function (xSemaphoreGiveFromISR) in the isr handler 
    */
#ifdef FSL_RTOS_FREE_RTOS
    NVIC_SetPriority((IRQn_Type)LPUART1_IRQn, configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY);
#endif
    UartInitEDMA();
    SartRingBufferEDMA();
