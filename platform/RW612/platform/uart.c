...
bool zbPlatUartReceiveChar(uint8_t *ch)
{
    ...
    if(DbgConsole_TryGetchar((char *)ch) == kStatus_Success)
    {
        ret = true;
#ifdef ZIGBEE_EVENT_IMPL
        /* When using the debug console, we don't have any callback mechanism, so we have to schedule at least one
         * more time the application to read again until no character is available */
        zbTaskletsSignalApp();
#endif
    }
