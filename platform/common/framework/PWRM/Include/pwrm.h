...
typedef struct _sWakeTimerEvent {
    TIMER_MANAGER_HANDLE_DEFINE(tmr);
    uint32 u32TickDelta;
    void (*prCallbackfn)(void);
} pwrm_tsWakeTimerEvent;

typedef pwrm_tsWakeTimerEvent PWR_tsWakeTimerEvent;
...
/* App function to register pre/wakeup callbacks */
void vAppRegisterPWRCallbacks(void);

PWRM_teStatus PWR_eScheduleActivity(pwrm_tsWakeTimerEvent *psWake,
                                    uint32 u32TimeMs,
                                    void (*prCallbackfn)(void));
